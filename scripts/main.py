...
def cat_cat_2d_morp(df_ip, x1, x2, y):
    ...
    return {
        ...
        "Unweigh_morp": df["unweigh_morp"].sum() / (df_ip[x1].nunique() * df_ip[x2].nunique()),
...
def main():
    ...
    if not cat_fet_prop_df.empty:
        cat_fet_prop_df = cat_fet_prop_df.sort_values(by="Weighted_morp", ascending=False).reset_index(drop=True)
        fig = px.bar(cat_fet_prop_df, x="Feature_nm", y="Weighted_morp", title="Categorical Feature Importance")
        fig.show()
    ...
    for i, j in itertools.combinations(cont_pred_list, 2):
        cont_cont_dict = {
            ...
            "Cont_1_morp_url": mp.morp_cont_resp_cont_pred(data_set, i, resp_type, response)["Plot_link"],
            "Cont_2_morp_url": mp.morp_cont_resp_cont_pred(data_set, j, resp_type, response)["Plot_link"],
        } if resp_type == "Continuous" else {
            "Cont_1": i,
            "Cont_2": j,
            "Correlation": pc.cont_pred_corr(data_set[i], data_set[j]),
            "Cont_1_morp_url": mp.morp_cat_resp_cont_pred(data_set, i, resp_type, response)["Plot_link"],
            "Cont_2_morp_url": mp.morp_cat_resp_cont_pred(data_set, j, resp_type, response)["Plot_link"],
        }
        cont_cont_list.append(cont_cont_dict)

    if len(cont_cont_list) >= 1:
        cont_cont_corr_df = pd.DataFrame(cont_cont_list)
        cont_cont_corr_df = cont_cont_corr_df.sort_values(by="Correlation", ascending=False).reset_index(drop=True)
        cont_cont_corr_df = cont_cont_corr_df[cont_cont_corr_df["Cont_1"] != cont_cont_corr_df["Cont_2"]]
        cont_cont_corr_html_plt = pc.heatmap_plot_corr(cont_cont_corr_df, "Cont_1", "Cont_2", "Corr")
    else:
        cont_cont_corr_df = pd.DataFrame(cont_cont_list)
    ...
    cat_cont_list = [(a, b, pc.cat_cont_corr(data_set[a], data_set[b]),
    ...
    if len(cat_cont_list) >= 1:
        ...
        cat_cont_corr_html_plt = pc.heatmap_plot_corr(cat_cont_corr_df, "Cont", "Cat", "Corr")
    ...
    if len(cat_pred_list) > 1:
        cat_pairs = [(x, y) for i, x in enumerate(cat_pred_list) for j, y in enumerate(cat_pred_list) if i < j]
        cat_cat_2d_morp_df = pd.DataFrame(cat_pairs, columns=['Cat_1', 'Cat_2'])
        cat_cat_2d_morp_df[['Weighted_morp', 'Unweighted_morp']] = cat_cat_2d_morp_df.apply(
            lambda row: pd.Series(cat_cat_2d_morp(data_set, row['Cat_1'], row['Cat_2'], response)), axis=1)
        cat_cat_2d_morp_df = cat_cat_2d_morp_df.merge(cat_cat_corr_t_df, on=['Cat_1', 'Cat_2'])
        cat_cat_2d_morp_df = cat_cat_2d_morp_df.merge(cat_cat_corr_v_df,
    ...
    if len(cat_pred_list) > 0 and len(cont_pred_list) > 0:
        ...
        cat_cont_2d_morp_list = [{"Cat": i, "Cont": j, **cat_cont_2d_morp(data_set, i, j, response)} for i in
                                 cat_pred_list for j in cont_pred_list if i != j]
        # convert the list of dictionaries into a DataFrame
        cat_cont_2d_morp_df = pd.DataFrame(cat_cont_2d_morp_list)

        cat_cont_2d_morp_df = pd.merge(cat_cont_2d_morp_df, cat_cont_corr_df, on=["Cat", "Cont"], how="left")
        # add a column with the absolute values of the correlation
        cat_cont_2d_morp_df["Correlation_Abs"] = cat_cont_2d_morp_df["Correlation"].abs()
        # sort the DataFrame by weighted_morp in descending order and reset the index
        cat_cont_2d_morp_df = cat_cont_2d_morp_df.sort_values("Weighted_morp", ascending=False).reset_index(drop=True)
    else:
    ...
    if len(cont_pred_list) > 1:
        cont_cont_2d_morp_df = pd.DataFrame(
            columns=["Cont_1", "Cont_2", "Weighted_morp", "Unweighted_morp", "Plot_link"])
        for i, j in itertools.combinations(cont_pred_list, 2):
            dict1 = cont_cont_2d_morp(data_set, i, j, response)
            cont_cont_2d_morp_df = cont_cont_2d_morp_df.append({"Cont_1": i, "Cont_2": j,
                                                                "Weighted_morp": dict1["Weighted_morp"],
                                                                "Unweighted_morp": dict1["Unweighted_morp"],
                                                                "Plot_link": dict1["Plot_link"]}, ignore_index=True)
        cont_cont_2d_morp_df = cont_cont_2d_morp_df.merge(cont_cont_corr_df, on=["Cont_1", "Cont_2"])
        cont_cont_2d_morp_df["Correlation_Abs"] = cont_cont_2d_morp_df["Correlation"].abs()
        cont_cont_2d_morp_df = cont_cont_2d_morp_df.sort_index(axis=1, ascending=True) \
    ...
    # styling of table
    table_styles = [
        {"selector": "", "props": [("border-collapse", "collapse"), ("border", "2px solid #ddd")]},
        {
            "selector": "th",
            "props": [
    ...
    cat_fet_prop_df = (
        cat_fet_prop_df
        .style
        .format({"Plot_link1": url_click, "Plot_link2": url_click, "Morp_plot_link": url_click})
    ...
    cat_cat_corr_t_df = cat_cat_corr_t_df.style.format({"Cat_1_morp_url": url_click, "Cat_2_morp_url": url_click}) \
        .set_table_styles(table_styles) \
        .hide_index()

    cat_cat_corr_v_df = cat_cat_corr_v_df.style.format({"Cat_1_morp_url": url_click, "Cat_2_morp_url": url_click}) \
        .set_table_styles(table_styles) \
        .hide_index()

    cat_cont_corr_df = cat_cont_corr_df.style.format({"Cat_morp_url": url_click, "Cont_morp_url": url_click}) \
        .set_table_styles(table_styles) \
        .hide_index()
